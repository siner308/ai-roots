...
description: "언어나 프레임워크 사이로 코드를 포팅·번역·재작성하거나, 기존 코드가 런타임에 무엇을 하는지 답할 때마다 적용한다 — 결과를 내놓기 전에 구체적인 입력으로 머릿속에서 돌려보고 실제로 무엇을 반환하는지 밝히되, 두 버전이 갈리는 경계 입력을 포함한다. 소스를 읽은 것과 동작을 아는 것은 다르다."
---
