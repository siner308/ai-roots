...
description: "여기서 볼 수 없는 대상과 통신하는 코드를 쓸 때마다 적용 — 외부 API, browser, 따옴표가 까다로운 shell 명령, 익숙하지 않은 library, 데이터 pipeline. 가장 작은 조각부터 돌려 실제 출력을 눈으로 확인한 뒤 그 위에 다음 조각을 얹는다. 전체 스크립트를 다 써놓고 한꺼번에 돌리지 않는다."
---
...
3. **한 번에 한 겹씩 쌓아라.** N단계가 성공한 뒤에야 N+1단계를 추가하세요. N+1이 실패하면 고치고 1단계부터 다시 검증하세요 — 고치는 과정에서 앞 단계가 깨졌을 수 있다고 보세요.
