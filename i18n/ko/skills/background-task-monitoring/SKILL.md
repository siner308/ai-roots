...
---
  ...
  적용 대상: 단발 빌드, 테스트 스위트, 일회성 마이그레이션, 가치가 과정이
  아니라 최종 결과에 있는 모든 작업.
  ...
  적용 대상: codex `--json` 리뷰(`/review` 스킬 — 이벤트 스트림이 codex가 들여다보는 파일과 reasoning을 실시간으로 보여준다), 단계 경계가 있는 data pipeline, 긴 seed 스크립트, crawler, 실행 도중의 단계
  ...
  5분짜리 Codex 리뷰는 인터벌 폴링(Rung 3) 대상이 아니다 — 하지만 `--json`이 이벤트를 스트림하므로, Rung 2가 codex가 뭘 들여다보는지 폴링 비용 없이 보여준다. 12단계
