...
에스컬레이션은 실패가 아니라 규칙의 일부다. 약한 모델을 고집스럽게 밀어붙이면 hysteresis로 이어진다 — 틀린 방향이 그대로 굳어 버린다. 사다리에는 Opus 위 한 칸이 있다: Opus 자체가 (spec 모호함이 아니라) 정말 어려운 추론에서 막히면 **Fable 5**로 에스컬레이션한다 — 비용 때문에 그 예외적 경우에만.
