...
**유일한 예외 — 레포의 PR 템플릿**(`.github/pull_request_template.md` 또는 `.github/PULL_REQUEST_TEMPLATE/`): 정의된 섹션을 그 순서대로 따르되, 각 섹션을 캡의 취지만큼 간결하게 채우고, 해당 없는 섹션은 "N/A" 채우기 대신 빼버린다. 포맷 규칙(ASCII 불릿, 백틱 코드 참조, API 전달)은 그 안에서도 그대로 적용된다.
