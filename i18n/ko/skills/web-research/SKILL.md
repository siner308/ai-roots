...
**웹 브라우징의 기본 도구는 WebSearch/WebFetch가 아니라 `agent-browser`입니다.** 사이트를 방문하거나, 데이터를 추출하거나, 페이지와 상호작용하는 작업이면 agent-browser를 먼저 적극적으로 쓰세요. WebSearch보다 앞서 첫 도구로 고르세요.
...
이 fallback은 비싸게 배운 것입니다. 어떤 분석을 팩트체크하면서 어려운 수치 둘(한 회사의 연차 공시에 있는 지역별 매출 비중과 업계 단체의 출하량 전망)을 확인해야 했습니다:
