---
name: web-research
description: "웹을 둘러보거나, 페이지 내용을 추출하거나, 데이터를 긁거나, 사이트에서 수치를 가져올 때 적용하세요 — agent-browser(기본 lightpanda, 무겁거나 상호작용이 필요한 페이지는 chrome), 발견용 WebSearch, 차단 본문 회수용 insane-search 스킬 중에서 고릅니다. 핵심은 차단 신호 처리입니다. 페이지가 봇 차단되면 브라우저 엔진을 더 높이지 말고 — 사실만 필요하면 WebSearch, 차단된 페이지의 원문 전체가 필요하면 insane-search에 위임하세요. Codex --search는 교차검증 전용 GPT 의존 최후 수단입니다."
---
...
2. 차단 신호 + 값만 필요 → 필요한 정확한 수치를 박은 키워드 쿼리로 `WebSearch` (예: `Arm Holdings 20-F China revenue percent 2026`). 스니펫을 읽으세요 — 숫자가 그대로 들어있는 경우가 많습니다.
