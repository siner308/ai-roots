...
교정을 즉시 받아들여라. 지금 작업을 고쳐라. 가드레일을 논하느라 작업을 막지 마라.
...
- 사용자 확인 없이 가드레일을 자동으로 쓰지 않는다
- 대화의 모든 디테일을 포착하지 않는다 — 미래의 실수를 막는 교정만
- memory 시스템을 대체하지 않는다 — memory는 맥락을 추적하고, 가드레일은 동작을 강제한다
