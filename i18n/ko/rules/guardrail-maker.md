...
**Writing location:** harness가 읽는 표면은 전부 이 저장소에서 설치된 것이다 — 심링크이거나 생성된 블록이지 원본이 아니다. 실제 경로를 먼저 풀고(`readlink -f ~/.claude/rules/ai-roots`로 닿는다), 상주 규칙은 git이 추적하는 트리(`rules/...`)에 써라. 설치된 사본에는 절대 쓰지 마라 — 정본은 repo이고, 생성된 블록은 다음 설치 때 덮어써진다. 상황별 skill은 같은 repo의 `skills/<name>/SKILL.md`에 두고, 각각 `name`과 트리거 중심 `description`을 담은 YAML frontmatter를 붙인다. `install.sh`를 다시 돌리면 지원하는 모든 harness로 전파된다.
