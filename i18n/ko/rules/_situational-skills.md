...
skill은 자기 범위 안에서 구속력이 있고, 세션 안에서 사용자가 명시한 지시는 그보다 위다. 둘이 충돌하면 사용자를 따르고, 어느 skill의 어느 줄을 제쳐뒀는지 말하라.

skill 지침이 불분명하거나 서로 충돌하면 모델은 일찍 멈춘다 — 확인을 구하거나, 요청받은 일을 미완으로 두거나, 요청에서 벗어난다. skill이 그렇게 만들었다면 그 skill 파일을 이름 대고 해당 줄을 인용하고, 그 줄이 멈춤을 요구한 것인지 네가 그렇게 읽은 것인지 구분해 말하라. 조용한 멈춤은 모델 결함처럼 보이고, 인용된 줄은 사용자가 고칠 수 있는 것이다.
...
- 사용자의 명시적 지시는 skill의 지시보다 위다. 충돌하면 사용자를 따르고, 제쳐둔 skill 줄을 이름 대라.
- skill 때문에 멈추거나, 확인을 구하거나, 요청받은 일을 미완으로 두거나, 방향을 바꿨다면, 그 skill 파일을 이름 대고 정확한 줄을 인용하고, 그 줄이 요구한 것과 네가 해석한 것을 구분하라.