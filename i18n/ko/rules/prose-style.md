...
끊는 자리가 네 몫인 곳에서, 두 가지 경우에 적용된다: 실제 너비 제한이 끊김을 강제하는 곳(코드 주석, 커밋 메시지 본문, 고정폭 텍스트), 그리고 끊김 자체가 렌더링되는 곳 — Markdown/MDX의 `\`나 `<br>` 등, 독자가 실제 줄바꿈으로 보게 되는 것. soft-wrap 되는 prose(Markdown, 채팅)는 *소스 수준의* 하드 줄넘김이 아예 필요 없다: 그냥 흘려보내라, 한 줄에 한 문장. 이미 스스로 흐르는 텍스트에 수동 줄넘김을 끼워 넣지 마라.
...
- soft-wrap 되는 prose(Markdown, 채팅)는 소스 수준의 하드 줄넘김을 넣지 마라 — 문장을 줄 중간에서 쪼개지 말고, 그냥 흘려보내라. rendered break(`\`, `<br>`)는 감김이 아니라 presentation 선택이다: 흐름이 멈추는 지점에는 허용, 문장 중간에는 금지.
