...
글이 독자에게 닿는 방식은 세 가지 별개의 선택으로 갈린다: 문장 *안의 단어*, *줄을 어디서 끊느냐*, 그리고 *주장의 출처를 어디에 두느냐*. 셋 다 명료한 사고를 기계 같은 출력으로 조용히 바꿔놓을 수 있고, 망가지는 방식이 서로 다르다 — 그래서 이 규칙은 셋 다 다룬다.
...
독자가 출처를 찾는 순간은 어떤 주장이 의심스러워지는 그 순간이다. 링크를 바로 거기 — 주장을 담은 문장, 리스트 항목, 표 셀, 인용문 머리 — 에 두면 의심이 생긴 자리에서 바로 풀린다. 문서 끝에 몰아둔 참고 자료 블록은 그걸 다른 곳에서 푼다: 독자는 주장을 떠나 목록을 훑고, 어느 항목이 어느 문장을 뒷받침하는지 짐작한 뒤 되돌아와야 한다. 문서 전체에 목록 하나만 있으면 빈틈도 가려진다 — 어느 주장에 출처가 있고 어느 주장에 없는지 아무도 알 수 없다.

### 출처를 붙이는 법

- **문장이나 리스트 항목**은 그것을 뒷받침하는 링크로 끝난다: `습도가 85%를 넘으면 돔이 닫힌다 — [운영 매뉴얼](url).` 한 항목이 주장을 여럿 하면 주장마다 자기 링크를 단다.
- **인용문**은 인용 텍스트 앞, 머리에서 출처를 밝힌다. 그래야 독자가 누구의 말이 이어지는지 알고 읽는다.
- **표**는 주장을 담은 셀에서 링크한다 — 행마다 그 기록으로 이어지는 식별자 열이나 출처 열 — 또는 출처 하나가 표 전체를 덮으면 caption에서. 표 아래 산문에만 적힌 출처는 모든 행을 출처 없는 상태로 남긴다.
- **같은 문서 안 다른 곳에 있는 근거**(스크린샷, 부록, 캡처한 페이지)에는 `id`를 주고 anchor로 링크한다: `[캡처](#fig-closures)`. 말로 적은 위치 지시(`4절의 캡처 참고`)는 변장한 참고 자료 블록이다 — 독자는 여전히 찾아가야 한다.
- **deep link가 없는 출처**(검색 폼 뒤의 기록, 어떤 조회든 URL이 하나인 페이지)는 가장 가까운 안정된 페이지를 링크하고 경로를 말로 적는다: `[등록 시스템](url) → 검색 → 건물명 → 기록`. 링크가 닿지 않는 범위를 말해라 — 홈 링크를 출처처럼 내놓으면 독자를 속인다.

### 건드리지 않는 것

- 본문에 마커가 있는 각주(`[^3]`가 주장 옆에, 주석은 끝에). 마커가 사용 지점에 있고, 주석이 어디에 렌더링되는지는 형식의 선택이다.
- 특정 주장을 뒷받침하지 않는 자료를 모은 마무리 읽을거리 목록. 그건 인용이 아니라 읽을거리 목록이고, 문서의 출처가 나타나는 유일한 자리가 되어서는 안 된다.

### 예시

❌ 출처는 끝에 몰려 있고, 위치 지시 하나는 말로 적혀 있다:
...
- `korean-style`은 이 규칙의 한국어 확장이다: AI 한국어의 tell(쉼표 습관, 음차 외래어, 번역투, 문장 리듬)을 구체적인 ❌/✅ 예시로 짚는다. 한국어를 쓸 땐 둘 다 적용한다 — 언어 공통 리듬은 이 규칙, 한국어에만 있는 tell은 그 규칙.
- `english-style`은 같은 확장의 영어판이다: AI 영어의 tell(닳은 어휘, em dash 쌓기, `not just X, it's Y` 틀, 접속어 패딩)을 같은 ❌/✅ 방식으로 짚고, 대본과 스크립트용 말하기 register 지침을 담는다. 영어를 쓸 땐 둘 다 적용한다.
- `thinking-expansion`은 사고용 어휘를 활성화한다. 이 규칙은 그 어휘가 독자에게 진짜로 도움 되지 않는 한 출력에 못 나오게 막는다. 둘이 반대 방향으로 당길 때, 출력 경계에서는 이 규칙이 이긴다.
- `grounded-assertions`는 주장에 근거가 있는지를 정하고, 여기 출처 배치 절은 그 근거가 산출물 안 어디에 놓이는지를 정한다. 앞은 통과하고 뒤에서 걸리는 주장은 출처는 있는데 찾을 수 없는 주장이다.
...
- 구어체 리듬은 대화체·설명체 산문에서만 기본이다. 구조화된 산출물은 자기 어조를 유지한다.
...
- 출처는 주장이 나오는 자리에 붙여라 — 문장이나 리스트 항목 끝, 표 셀이나 caption, 인용문 머리 — 문서 끝 참고 자료 블록이 아니라. 같은 문서 안 다른 곳의 근거는 말로 가리키지 말고 anchor로 링크하고, deep link가 없는 출처는 가장 가까운 안정된 페이지에 경로를 말로 덧붙여라.
