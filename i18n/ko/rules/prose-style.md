...
- **구어체 리듬** (소리 내어 말할 법한 완결된 문장) — **대화체·설명체 산문에만** 적용된다. 구조화된 산출물은 자기 어조를 유지한다 — 억지로 구어체로 만들지 마라.
...
끊는 자리가 네 몫인 곳에서, 두 가지 경우에 적용된다: 실제 너비 제한이 끊김을 강제하는 곳(코드 주석, commit 메시지 본문, 고정폭 텍스트), 그리고 끊김 자체가 렌더링되는 곳 — Markdown/MDX의 `\`나 `<br>` 등, 독자가 실제 줄바꿈으로 보게 되는 것. soft-wrap 되는 산문(Markdown, 채팅)은 *소스 수준의* 하드 줄넘김이 아예 필요 없다: 그냥 흘려보내라, 한 줄에 한 문장. 이미 스스로 흐르는 텍스트에 수동 줄넘김을 끼워 넣지 마라.
...
- soft-wrap 되는 산문(Markdown, 채팅)은 소스 수준의 하드 줄넘김을 넣지 마라 — 문장을 줄 중간에서 쪼개지 말고, 그냥 흘려보내라. rendered break(`\`, `<br>`)는 감김이 아니라 presentation 선택이다: 흐름이 멈추는 지점에는 허용, 문장 중간에는 금지.
- 파일에 원래 있던 hard-wrap 스타일은 너비 제한이 아니다. 린터나 포매터가 폭에 에러를 내지 않는 한 그 감김을 따라 하지 말고, 문장이 줄에 걸쳐 쪼개지지 않게 손대는 문단을 다시 흘려 써라. 문장이 끝난 뒤에 줄을 바꾸는 것은 허용이지 의무가 아니다.
