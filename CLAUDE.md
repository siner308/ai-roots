...
- A hook's *block message* is the exception: the user reads it on screen, so it may be written in the user's language. Prefer keeping it English and letting `hook_lang.localize` append the relay instruction (`/hook-lang` picks the language) — that keeps one copy of the text. A message already authored in another language stays as it is.
...
- A skill that ships a script invokes it by its installed path: `~/.claude/skills/<skill-name>/scripts/<file>`. `install.sh` links every skill folder there, so that path is what the install guarantees, and it holds wherever the session's working directory happens to be. A `scripts/…` relative path only resolves when the cwd already is the skill, and a `<SKILL_DIR>` placeholder in a shell snippet is substituted by nothing at run time — both leave the reader to guess. This is the one place a `~`-anchored absolute path belongs; repo content otherwise stays relative to the repo root.
...
| Hook message language | — | `hook_lang.py` (relay instruction) | `hook-lang` |
