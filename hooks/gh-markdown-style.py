...
Fail-open: any parse/IO error or unrecognized shape exits 0 (allow), so it never wedges unrelated Bash. It blocks (exit 2) only on a positively-identified problem.
"""
import json
import os
import re
...
API_TARGET_RE = re.compile(r"/(?:pulls|issues)\b")
MARKDOWN_RE = re.compile(r"(?m)^\s*[-*+]\s|`|^\s*#{1,6}\s|^\s*\[[ xX]\]|\[[^\]]+\]\([^)]+\)")
# 5+ trailing spaces = renderer padding; a real markdown hard break is exactly two, so intent is not caught.
RENDERER_RE = re.compile(r"(?m)^\s*•\s| {5,}$")

CLI_BODY_FLAGS = {"-b", "--body"}
CLI_BODY_FILE_FLAGS = {"-F", "--body-file"}
...
def cli_body(tokens):
    inline = flag_value(tokens, CLI_BODY_FLAGS)
    if inline is not None:
        return inline
    path = flag_value(tokens, CLI_BODY_FILE_FLAGS)
...
def api_body(tokens):
    for tok in tokens:
        if tok.startswith("body="):
            ...
            if val.startswith("@"):
                ref = val[1:]
                return read_file(ref) if ref != "-" else None
            return val
    return None
...
def main():
    ...
    is_cli = bool(GH_CLI_RE.search(cmd))
    is_api = bool(GH_API_RE.search(cmd)) and bool(API_TARGET_RE.search(cmd))
    if not (is_cli or is_api):
        return 0
    ...
    body = cli_body(tokens) if is_cli else api_body(tokens)
    if body is None or body.strip() == "":
        return 0

    if RENDERER_RE.search(body):
        block(
            "본문에 렌더러 아티팩트(`•` 불릿 또는 고정폭 후행 공백)가 있습니다 — "
