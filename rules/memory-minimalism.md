...
- **Conventions, principles, behavioral rules** — these belong in `rules/` (universal) or project `CLAUDE.md` (project-specific). Both are version-controlled and identical on every device.
...
1. Could this be a rule in `rules/`? → write a rule instead.
