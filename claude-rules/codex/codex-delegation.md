...
> **Applicability** — This rule loads only when OpenAI Codex is installed alongside Claude Code (`./install.sh --with-codex`). Without Codex, `Agent` with a distinct `subagent_type` substitutes for `/codex:adversarial-review` — the cross-model diversity is weaker, but the generator-vs-evaluator separation still holds. The in-platform delegation in `model-effort-delegation.md` applies either way.
...
Choose the narrowest mode that finishes the job.

**Reasoning effort:** Always `xhigh`. Pass `-c model_reasoning_effort=xhigh` on every invocation.
...
| Need | Command | Why |
|------|---------|-----|
| Independent code review | `/codex:diff-review` | Read-only reviewer; no implementation drift |
| Security-sensitive review | `/codex:adversarial-review` | Cross-family gate for auth, data, network, secrets, trust boundaries |
| Current docs or web research | `/codex:research` | Web-backed research without write access |
| Stuck after three failed attempts | `/codex:rescue` | Fresh reasoning stack without changing files |
| Bounded implementation | `/codex:autopilot` | Workspace edits allowed; risky actions still request approval |
| Unattended long implementation | `/codex:overnight` | Avoids approval stalls while preserving workspace sandbox |
| Explicit no-sandbox run | `/codex:yolo-overnight` | Only when the user explicitly accepts no sandbox/no approvals |

Each command file in `.claude/commands/codex/` carries the exact invocation. Do not duplicate the flag set here.
...
**Three-turn cap on stuck problems.** Do not attempt a 4th inline turn on the same hypothesis. Delegate to `/codex:rescue` with all ruled-out hypotheses included.

**Adversarial review on security-sensitive changes.** After every behavioral change touching authentication, authorization, database writes, network boundaries, secret handling, or trust boundaries, invoke `/codex:adversarial-review`. Read-only reads or pure internal refactors do not trigger. The reviewer persona — skeptical, security-first, classifies findings P0–P3, returns `VERDICT: SAFE` only when no critical issues surface at high coverage — lives in `.claude/agents/adversarial-reviewer.md` and is piped via stdin by the command.
...
4. **After Turn 3** — `/codex:rescue` with original task, each hypothesis attempted and why it failed, minimal reproducer, and ruled-out files.
...
**Stdin-piping commands** (`adversarial-review`, `rescue`, `overnight`, `research`, `yolo-overnight`, `autopilot`) take their prompt via stdin. Write to a temp file first, then redirect:
...
- Never skip `/codex:adversarial-review` on security-sensitive paths to save tokens.
- When invoking `/codex:rescue`, include ruled-out hypotheses so Codex does not redo the same work.
