...
라우팅 규칙 (mode selection, 어려운 문제 3-턴 cap, 보안 민감 경로 adversarial review, research, overnight work, 이미지 생성 등 capability 라우팅)은 `claude-rules/roots/model-effort-delegation.md` §Cross-Provider Delegation (Codex)에 있습니다. 이것은 **Claude 측** 규칙 — 언제 Codex를 호출할지를 Claude에게 알려주는 것이고, Codex의 동작 방식을 규정하지 않습니다.
