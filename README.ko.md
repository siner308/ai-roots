...
- `comment-discipline.py` — `Edit|Write|MultiEdit`에 걸리는 `PostToolUse` hook. 코드 파일에 **새로 추가된** 주석 줄을 감지(기존 주석 제외)해 `comment-discipline` 허용 목록과 대조하도록 다시 띄웁니다. 여러 줄 주석이면 각 줄바꿈이 의미 경계에 있는지도 모델에게 검사시킵니다.
...
- `prose-discipline.py` — `Edit|Write|MultiEdit`에 걸리는 `PostToolUse` hook. `comment-discipline`의 비코드 짝입니다. Markdown에서 문장 중간 하드 줄바꿈을 표시하고(줄바꿈은 문장이 끝나는 곳에만, 린터가 폭을 강제하는 경우 제외), 문장 수 게이트를 넘으면 추가된 산문에 간결함 재검사를 요구합니다.
